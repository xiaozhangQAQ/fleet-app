	...
	export default {
		...
		methods: {
			...
			setHours() {
				...
				setTimeout(()=> {
					this.$set(this.value, this.value.length - 3, this.getIndex(this.hours, this.hour));
				}, 8);
			...
			setMinutes() {
				...
				setTimeout(()=> {
					this.$set(this.value, this.value.length - 2, this.getIndex(this.minutes, this.minute));
				}, 8);
			},
			setSeconds() {
