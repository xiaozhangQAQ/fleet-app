		...
		<view :class="[direction==='column'?'uni-steps__column':'uni-steps__row']">
			<view :class="[direction==='column'?'uni-steps__column-text-container':'uni-steps__row-text-container']">
				<view v-for="(item,index) in options" :key="index" :class="[direction==='column'?'uni-steps__column-text':'uni-steps__row-text']">
					...
					<text :style="{color:index<=active?activeColor:deactiveColor}" :class="[direction==='column'?'uni-steps__column-desc':'uni-steps__row-desc']">{{item.desc || item.createTime}}</text>
					<view class="uni-steps_content" v-if="hascontent">			
						<view class="uni-steps_content-title">
							{{item.nodeType || '-'}}
						</view>
						<view class="uni-steps_content-item">
							<text class="uni-steps_content-item-label">操作人：</text>
							<view class="uni-steps_content-item-name">{{item.creater || '-'}}</view>
						</view>
						<view class="uni-steps_content-item">
							<text class="uni-steps_content-item-label">结果：</text>
							<view class="uni-steps_content-item-name">{{item.result || '-'}}</view>
						</view>											
					</view>
				</view>
			</view>
...
<style scoped>
	...
	.uni-steps_content-item{
		display: flex;
		padding: 20rpx;
		font-size: 28upx;
		color: #999;
	}
	.uni-steps_content-item-label{
		
	}
	.uni-steps_content-item-name{
		flex: 1;
	}
</style>