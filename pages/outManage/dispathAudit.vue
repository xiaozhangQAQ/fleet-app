<template>
	<view class="content">
		<view class="header_navbar" :style="{height:navHeight}">
			<uni-nav-bar title="调度审核" statusBar="true" background-color="#fff">
				<view slot="left">
...
<script>
	...
	import {dispatchApprovalList} from '@/api/api.js'
	import {
	    mapState 
	} from 'vuex'; 
	export default {
		...
		methods: {
			...
			toCheck(id){//去审核
				uni.navigateTo({
					url:'./dispathAuditDetail?orderId='+id+'&accountId='+this.userInfo.accountId
				})
			},
			initData(){
				this.pageNum = 0;
			...
			setDate() {
				...
				this.pageNum += 1;
				dispatchApprovalList({page:this.pageNum,accountId:this.userInfo.accountId}).then(res=>{
					var newdata = [];
