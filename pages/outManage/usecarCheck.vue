...
<script>
	...
	import {approvalFacadeList} from '@/api/api.js'
	import {
	    mapState 
	} from 'vuex'; 
	export default {
		...
		methods: {
			...
			toCheck(id){//去审核
				uni.navigateTo({
					url:'./usecarCheckDetail?orderId='+id+'&accountId='+this.userInfo.accountId
				})
			},
			initData(){
				this.pageNum = 0;
			...
			setDate() {
				...
				this.pageNum += 1;
				approvalFacadeList({page:this.pageNum,accountId:this.userInfo.accountId}).then(res=>{
					var newdata = [];
