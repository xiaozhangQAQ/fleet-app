...
<script>
	import {dispatchApprovalDetail,dispatchApprovalSubmit} from '@/api/api.js'
	export default {
		data() {
			return {
				sh:'',//是否通过
		...
		methods:{
			...
			upto(){//提交
				...
				dispatchApprovalSubmit({accountId:this.accountId,fkOrderId:this.fkOrderId,approvalType:'2',approvalResult:shIndex,approvalOpinion:this.ideal}).then(res=>{
					uni.navigateBack()
				})
			},
			initData(accountId,fkOrderId){
				dispatchApprovalDetail({accountId:accountId,orderId:fkOrderId}).then(res=>{
