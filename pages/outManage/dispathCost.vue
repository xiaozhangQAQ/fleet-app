<template>
	<view class="content">
		...
		<view class="apply_box" :style="{paddingTop:navHeight}">
			<block v-if="totalNum">
				<view class="apply_item" v-for="(item,index) in data" :key="index" @click="toDet(item.fkOrderId)">
			...
			<uni-load-more :status="upStatus" v-if="this.showLoadMore"/>
			<block v-if="isnull">
...
<script>
	...
	import {chargeCusFacadeList} from '@/api/api.js'
	import {
	    mapState 
	} from 'vuex'; 
	export default {
		...
		onReachBottom() {
			...
			if (this.max >= this.totalNum) {
				...
				console.log(this.max,this.totalNum,this.upStatus)
				return;
			}
		...
		methods: {
			...
			toCheck(id){//去审批
				uni.navigateTo({
					url:'./dispathCostDetail?orderId='+id+'&accountId='+this.userInfo.accountId
				})
			},
			initData(){
				this.pageNum = 0;
			...
			setDate() {
				...
				this.pageNum += 1;
				chargeCusFacadeList({page:this.pageNum,accountId:this.userInfo.accountId}).then(res=>{
					var newdata = [];
