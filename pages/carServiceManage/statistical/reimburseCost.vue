		...
		<view class="statistical_box">
			<view class="statistical_box-content">
				<view class="statistical_item">
					用车人数 <text>{{datas.personCount}}</text>
				</view>
				<view class="statistical_item">
...
<script>
	...
	import {claiminCostStatistics} from '@/api/api.js'
	import {
	    mapState 
	} from 'vuex'; 
	export default {
		...
		methods:{
			...
			inidata(){
				claiminCostStatistics({accountId:this.userInfo.accountId,date:this.usedate,fkOrgId:this.departs.id || ''}).then(res=>{
					this.datas = res.data;
				})
			}
		}
